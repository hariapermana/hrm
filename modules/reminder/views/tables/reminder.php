...
foreach ($rResult as $aRow) {
    $row = [];
    $numberOutput = '<a href="javascript:void(0);" onclick="init_reminder(' . $aRow['id'] . '); return false;">' ._d($aRow['date']) . '</a>';
    ...
    $row[] = $numberOutput;
    $assigned_to = '';
    if(!empty($aRow['assigned_to'])){
        $iStaff = $this->ci->staff_model->get($aRow['assigned_to']);
        $assigned_to = '<a data-toggle="tooltip" data-title="' . $iStaff->full_name . '" href="' . admin_url('profile/' . $aRow['assigned_to']) . '">' . staff_profile_image($aRow['assigned_to'], [
            'staff-profile-image-small',
        ]) . '</a>';
        $staff .= '<span>' . $iStaff->full_name . '</span>';
