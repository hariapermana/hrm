...
if (!$CI->db->field_exists('subject', db_prefix() . 'reminders'))
{
  $CI->db->query('ALTER TABLE `tblreminders`
   ADD `subject` varchar(225) NOT NULL;');
} 
if (!$CI->db->field_exists('start_periode_date	', db_prefix() . 'reminders'))
{
  $CI->db->query('ALTER TABLE `tblreminders`
   ADD `start_periode_date` date NOT NULL;');
} 
if (!$CI->db->field_exists('end_periode_date', db_prefix() . 'reminders'))
{
  $CI->db->query('ALTER TABLE `tblreminders`
   ADD `end_periode_date` int(11) NOT NULL;');
} 
if (!$CI->db->field_exists('status', db_prefix() . 'reminders'))
{
  $CI->db->query('ALTER TABLE `tblreminders`
...
{
  $CI->db->query("ALTER TABLE `". db_prefix() . "clients` ADD is_supplier tinyint(1)  DEFAULT 0 NOT NULL;");
}
