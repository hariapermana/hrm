...
define('PERFORMANCE_MODULE_NAME', 'performance');
...
function goals_global_search_result_output($output, $data)
{
    if ($data['type'] == 'performance') {
        $output = '<a href="' . admin_url('performance/goal/' . $data['result']['id']) . '">' . $data['result']['subject'] . '</a>';
    }
...
{
    $CI = &get_instance();
    $CI->load->model('performance/goals_model');
...
*/
register_activation_hook(PERFORMANCE_MODULE_NAME, 'goals_module_activation_hook');
...
*/
register_language_files(PERFORMANCE_MODULE_NAME, [PERFORMANCE_MODULE_NAME]);
